...
class AlgorithmTracer:
    ...
    def start(self) -> None:
        """トレーシングを開始"""
        if self._is_tracing:
            warnings.warn("Tracing is already running", UserWarning, stacklevel=2)
            return
    ...
    def save(self, file_path: Union[str, Path], format: str = "json") -> None:
        ...
        if format == "json":
            try:
                ...
                raise OSError(f"Failed to save trace data to {file_path}: {str(e)}") from e
        elif format == "pickle":
            try:
    ...
    def load(self, file_path: Union[str, Path], format: str = "json") -> None:
        ...
        if format == "json":
            try:
                ...
                raise OSError(f"Failed to load trace data from {file_path}: {str(e)}") from e
        elif format == "pickle":
            try:
