...
class GCSBackend(RemoteBackend):
    ...
    def exists(self, rel_path: str) -> bool:
        # google-cloud-storage はスタブを持たないため戻り値が Any になる
        return cast(bool, self._bucket.blob(self._key(rel_path)).exists())

    def read_bytes(self, rel_path: str) -> bytes:
        return cast(bytes, self._bucket.blob(self._key(rel_path)).download_as_bytes())
...
class HTTPBackend(RemoteBackend):
    ...
    def list_files(self, prefix: str = "") -> list[str]:
        ...
        files: list[str] = list(manifest.get("files", []))
        if prefix:
