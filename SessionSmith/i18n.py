...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    "ja": {
        ...
        "error.ssm_remove_failed": "既存の .ssm の削除に失敗しました: '{path}'. 理由: {reason}. 他のプロセスが使用中でないか確認してください",
        "error.ref_case_conflict": "{kind} '{name}' は既存の '{existing}' と大文字小文字しか違いません。macOS や Windows では同じファイルになるため作成できません",
    ...
    "en": {
        ...
        "error.ssm_remove_failed": "Failed to remove the existing .ssm directory: '{path}'. Reason: {reason}. Check that no other process is using it",
        "error.ref_case_conflict": "{kind} '{name}' differs only in case from the existing '{existing}'. They would be the same file on macOS and Windows",
